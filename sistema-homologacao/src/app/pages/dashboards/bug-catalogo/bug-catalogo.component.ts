    ...
    <div class="dashboard-content">
      ...
      <div *ngIf="!loading && !error" class="content">
        ...
        <div class="dashboard-section-header">
          <h2>📚 Bugs - Catálogo</h2>
          <p class="description">Bugs identificados em componentes do catálogo geral</p>
        </div>
        ...
        <div class="infografia-container">
          ...
          <div class="infografia-card danger">
            ...
            <div class="infografia-unit">No catálogo</div>
          </div>

          <!-- Card Componentes -->
          <div class="infografia-card info">
            <span class="infografia-icon">📦</span>
            <div class="infografia-value">{{ componentCount }}</div>
            <div class="infografia-label">Componentes</div>
            <div class="infografia-unit">Com bugs</div>
          </div>
        ...
        <div class="categories-grid">
          <div class="category-card" *ngFor="let item of catalogBugsData" [ngClass]="'status-' + item.status">
            <div class="category-header">
              <span class="category-icon">{{ item.icon }}</span>
              <div class="category-titles">
                <span class="category-name">{{ item.component }}</span>
                <span class="category-parent">{{ item.catalog }}</span>
              </div>
            </div>
        ...
        <div class="table-container">
          ...
          <div class="table-responsive">
            <table class="table-data">
              ...
              <tbody>
                <tr *ngFor="let item of catalogBugsData">
                  <td>
                    <span class="category-badge">{{ item.icon }} {{ item.component }}</span>
                  </td>
                  <td>{{ item.catalog }}</td>
...
export class BugCatalogoComponent implements OnInit, OnDestroy {
  ...
  catalogBugsData: CatalogBugData[] = [];
  ...
  ngOnInit(): void {
    ...
    this.dashboardService.getBugStats().pipe(takeUntil(this.destroy$)).subscribe({
      ...
      error: (err) => {
        if (!this.catalogBugsData.length) {
  ...
  private applyBugStats(bugs: Record<string, number>): void {
    ...
    this.componentCount = entries.length;
    this.criticalCount = entries.filter((entry) => entry.count >= 10).length;
    this.catalogBugsData = entries.map((entry) => {
      const status: CatalogBugData['status'] = entry.count >= 10 ? 'danger' : entry.count >= 5 ? 'warning' : 'info';
