...
@Component({
    ...
    <div class="dashboard-content">
      ...
      <div *ngIf="!loading && !error" class="content">
        ...
        <div class="dashboard-section-header">
          <h2>🐛 Bugs por Subcategoria</h2>
          <p class="description">Detalhamento detalhado de inconsistências agrupadas por subcategorias</p>
        </div>
        ...
        <div class="infografia-container">
          ...
          <div class="infografia-card danger">
            ...
            <div class="infografia-unit">Todas as subcategorias</div>
          </div>

          <!-- Card Subcategorias -->
          <div class="infografia-card info">
            <span class="infografia-icon">📂</span>
            <div class="infografia-value">{{ subcategoriesData.length }}</div>
            <div class="infografia-label">Subcategorias</div>
            <div class="infografia-unit">Identificadas</div>
          </div>

          <!-- Card Críticos -->
          <div class="infografia-card warning">
            <span class="infografia-icon">⚠️</span>
            <div class="infografia-value">{{ criticalCount }}</div>
            <div class="infografia-label">Críticos</div>
            <div class="infografia-unit">Requerem atenção imediata</div>
          </div>
        ...
        <div class="chart-container">
          ...
          <div class="chart-content">
            <!-- Aqui virá o gráfico -->
            <div style="display: flex; align-items: center; justify-content: center; height: 300px; color: #95a5a6;">
              <p>Gráfico será renderizado aqui</p>
            </div>
          </div>
        </div>
        ...
        <div class="categories-grid">
          <div class="category-card" *ngFor="let item of subcategoriesData" [ngClass]="'status-' + item.status">
            <div class="category-header">
              <span class="category-icon">{{ item.icon }}</span>
              <div class="category-titles">
                <span class="category-name">{{ item.subcategory }}</span>
            ...
            <div class="category-stats">
              ...
              <div class="stat">
                ...
                <span class="stat-label">do total</span>
              </div>
            </div>
        ...
        <div class="table-container">
          ...
          <div class="table-responsive">
            <table class="table-data">
              ...
              <tbody>
                <tr *ngFor="let item of subcategoriesData">
                  <td>
                    <span class="category-badge">{{ item.icon }} {{ item.subcategory }}</span>
                  </td>
                  <td>{{ item.category }}</td>
  ...
  styles: [`
    ...
    .infografia-card.danger .infografia-value {
      color: #e74c3c;
    }

    .infografia-card.success {
      border-left-color: #27ae60;
    }

    .infografia-card.success .infografia-value {
      color: #27ae60;
    }
    ...
    .infografia-card.warning .infografia-value {
      color: #f39c12;
    }

    .infografia-card.info {
      border-left-color: #3498db;
    }

    .infografia-card.info .infografia-value {
      color: #3498db;
    }
    ...
    .category-card.status-danger {
      border-top: 4px solid #e74c3c;
    }

    .category-card.status-success {
      border-top: 4px solid #27ae60;
    }

    .category-card.status-warning {
      border-top: 4px solid #f39c12;
    }

    .category-card.status-info {
...
export class BugSubcategoriaComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.dashboardService.getBugStats().pipe(takeUntil(this.destroy$)).subscribe({
      ...
      error: (err) => {
        if (!this.subcategoriesData.length) {
  ...
  private applyBugStats(bugs: Record<string, number>): void {
    ...
    this.criticalCount = entries.filter((entry) => entry.count >= 10).reduce((acc, entry) => acc + entry.count, 0);
    this.subcategoriesData = entries.map((entry) => {
      const status: SubcategoryData['status'] = entry.count >= 10 ? 'danger' : entry.count >= 5 ? 'warning' : 'info';
